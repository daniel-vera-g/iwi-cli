...
const mensa = async argv => {
    ...
    }).catch(err => console.error(`There was an error querying the Mensa menu. Error: ${err}`));
};
 ...
 */
const getData = () => {
    // TODO error handling if no reult or no internet
    return new Promise((response, reject) => {
        ...
        axios.get(url, {headers: {accept: "application/json"}}).then(res => {
            ...
            const getMealName = (mealGroupIndex, mealIndex) => mealGroups[mealGroupIndex].meals[mealIndex].name;
            const getMealPrice = (mealGroupIndex, mealIndex) => mealGroups[mealGroupIndex].meals[mealIndex].priceStudent;
